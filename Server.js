...
app.post("/webtoondelete", (req,res)=>{
    connection.query("DELETE FROM webtooninformation WHERE webtoon_name = ?",[req.body.webtoon_name],
    function(err,rows,fields){
        if(err){
            console.log("로드 실패");
        }else{
...
app.post("/noveladd", (req,res)=>{
    connection.query("INSERT INTO novelinformation (novel_name, novel_view, novel_rate, novel_link, novel_img) VALUES (?,?,?,?,?)",[req.body.novel_name,req.body.novel_view,req.body.novel_rate,req.body.novel_link,req.body.novel_img],
    function(err,rows,fields){
        if(err){
            console.log("로드 실패");
        }else{
...
app.post("/webtoonadd", (req,res)=>{
    connection.query("INSERT INTO webtooninformation (webtoon_name, webtoon_view, webtoon_rate, webtoon_link, webtoon_img) VALUES (?,?,?,?,?)",[req.body.webtoon_name,req.body.webtoon_view,req.body.webtoon_rate,req.body.webtoon_link,req.body.webtoon_img],
    function(err,rows,fields){
        if(err){
            console.log("로드 실패");
        }else{
